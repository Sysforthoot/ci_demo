...
describe('cc-app-test 1', () => {
  ...
  it("open the hoot medplus login page",()=>{
    ...
    cy.visit("https://hoot-web-staging.herokuapp.com/");  
       ...
       cy.wait(10000);
       cy.get("#claimNumber").type(keyword);
       cy.wait(2000); 
       cy.get("#claimantLastName").type(keyword);
       cy.wait(2000);
       cy.get("#insured").type(keyword);
      //  cy.wait(2000);
...
cy.wait(2000);
cy.get(':nth-child(3) > .flex > .radio-container > .selector').click(); // reason for decline
cy.wait(2000);
cy.get('#confirmationModalConfirmationButton').click(); // submit the cancel
cy.wait(5000); 
...
it("again accept the case",()=>{
  ...
  cy.get('li:nth-child(1) > .block > .flex').click(); //row 1
  cy.wait(15000);
  cy.get('#selectExpertButton').click(); // select ME 1
  cy.wait(2000);
  cy.get('#confirmProfessionalButton').click(); // confirmation modal
  cy.wait(15000);
  cy.get('#headerLogoutLink').click(); // logout
